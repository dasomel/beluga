...
LAKEKEEPER_PODS=$(kubectl get pods -n beluga-data -l app=lakekeeper --no-headers 2>/dev/null | grep -c "Running" || echo 0)
log_info "Active Lakekeeper pod(s): ${LAKEKEEPER_PODS}"

log_info "2. Checking SeaweedFS S3 storage pod..."
SEAWEED_PODS=$(kubectl get pods -n beluga-data -l app=seaweedfs --no-headers 2>/dev/null | grep -c "Running" || echo 0)
