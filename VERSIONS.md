...
| OpenLDAP | 2.6.14 (ldapium 정식 릴리스 전) | `ghcr.io/dasomel/ldapium:nightly-4e85165` | OpenLDAP Public License 2.8 / Apache-2.0 (ldapium image project) | 계정 원천 (D20) — **GHCR published image**. vegardit의 demo seed/`LDAP_INIT_*` 계약을 제거하고 ldapium의 명시적 `LDAP_ROOT_DN`/`LDAP_ADMIN_*`/`LDAP_SEED_DIR` 계약으로 전환. OpenLDAP 2.6.14 source-built image, first launch sample data 없음 |
| ldapium UI | (정식 릴리스 전) | `ghcr.io/dasomel/ldapium-ui:nightly-d1b7b9e` | Apache-2.0 | LDAP DIT browser 및 user/group CRUD 관리 UI — **GHCR published image**. Beluga의 LDAP 서버와 동일 ldapium release 계열. 현재 Beluga 매니페스트에서는 별도 UI 배포를 아직 활성화하지 않음 |
