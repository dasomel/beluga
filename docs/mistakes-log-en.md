...
| 2026-09-27 | ci | `check-version-consistency.py` treated a VERSIONS.md image absent from rendered output as `skipped`, allowing chart renames or removed images to pass CI. It rendered only default Helm values and missed the worker/OpenMetadata-enabled deployment combination. | Render both charts for every deployed values combination and require each declared image to match or have a reasoned explicit allowlist entry. Fail on missing images, version drift, stale allowlist entries, or allowlist entries removed from VERSIONS.md; print a table and run built-in self-tests. Allowlist only the intentionally undeployed optional ldapium UI, and remove the kubectl image entry from VERSIONS.md because the internal CA job uses the Python Kubernetes API client instead. |
