...
| 2026-09-24 | ci | `cleanup-merged-branch.yml` failed twice when the head ref was already gone: `gh pr merge --delete-branch` (2026-09-23 PR merge) and Dependabot's own branch deletion (2026-09-18) both deleted the ref first, then the workflow's own `DELETE /git/refs/heads/<ref>` call got a 404/422 "Reference does not exist" that `curl --fail-with-body` treated as failure. | Capture the HTTP status via `--write-out`, treat 204 as deleted and a 404/422 body containing "Reference does not exist" as already-deleted (`::notice::`), and keep every other non-204 status (401/403, etc.) failing the job. Verified live against the GitHub API (204/422/401) and actionlint-clean. |
