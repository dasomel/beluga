...
| 2026-09-27 | ci | (false-green) `tests/run-all.sh` omitted `tests/06-authz-defaults.sh`, so `make test` could miss regression of the default-deny rule for analyst SELECT on new tables. | Restored test 06 in numeric order and added an offline `make validate` gate requiring every numbered shell test exactly once in ascending order. Exclusions require a reason and are currently empty; built-in negative fixtures reject missing, duplicate, out-of-order, nonexistent, stale-exclusion, and reasonless-exclusion cases. |
