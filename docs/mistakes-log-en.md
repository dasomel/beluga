...
| 2026-09-24 | ci | D21(#117): `sast.yml`'s `trivy config` on `scan-ref: gitops` parsed chart *source* templates as raw YAML — false-green. Reproduced locally with `trivy 0.69.3`: 4 files affected — `06-trino.yaml` and `07-airflow.yaml`/`opa.yaml` truncated mid-file at a Go-template line (trino's coordinator/worker Deployment + Service fell past the parse error and were never scanned), and `11-openmetadata.yaml` fully skipped with no error at all (starts with `{{ if ... }}`). Resources gated behind non-default values that `scripts/gitops/01-argocd-bootstrap.sh` sets for the 48/64GB RAM profile (`openmetadata.enabled`, `trino.workerEnabled` — see `scripts/common/env.sh`) never rendered under a single default `helm template` either. | Render every chart+values combination actually deployed (32GB default: both flags false; 48/64GB: both true — confirmed the two flags always toggle together, no other combination exists) plus beluga-platform, then run `trivy config` on the rendered output. Verified: 0 parse errors, 3/3 rendered targets fully scanned (vs. 3 truncated + 1 fully skipped before). HIGH (non-blocking) rose 19→48 (previously-hidden resources now visible, expected); CRITICAL stayed at 1 (KSV-0041) with `.trivyignore.yaml`'s path updated to match the new scan root (`beluga-platform/manifests.yaml`) — no new CRITICALs after rendering. Reused the pinned Helm setup action SHA already used by `ci.yml`. Updated the CI stage-parity table in `docs/development.md`/`development-ko.md` for the renamed/added steps so `make validate` stays green. **Correction (same-day follow-up, GPT critic P2)**: the "no wider suppression" claim above was wrong — the exception, previously scoped to the single `apisix-gateway.yaml` file, had effectively widened to `beluga-platform/manifests.yaml` (the whole chart's concatenated rendered output), so any future KSV-0041 anywhere in beluga-platform would have been silently suppressed. Switched to `helm template --output-dir` so each source template renders to its own file (`rendered/beluga-platform/templates/apisix-gateway.yaml`, etc.) and re-scoped the ignore path back to that single template. Verified with a negative test: added a KSV-0041-triggering ClusterRole (secrets get/list) to a *different* template (`openldap.yaml`) in a scratch copy, re-rendered and scanned — reported CRITICAL (not suppressed), then discarded. Same critic pass (P2 #2) also found `gitops/apps/*.yaml` (the 3 ArgoCD Application manifests, plain K8s YAML) never pass through `helm template` and had silently dropped out of scanning once scan-ref moved from `gitops` to `rendered` — copied them into `rendered/plain/` to restore coverage (`Chart.yaml`/`values.yaml` intentionally excluded: Helm metadata/inputs, no `kind:`, not K8s objects). Re-verified: 0 parse errors (`09-ingress.yaml` intentionally empty post-APISIX-migration, correctly scans 0), CRITICAL 0 new findings (1 suppressed as intended), HIGH unchanged at 48. |
